...
static Collision* _instance = nullptr;
...
Box Collision::setBox(float _x, float _y, float _w, float _h, float _vx, float _vy)
{
	return Box(_x, _y, _w, _h, _vx, _vy);
}

bool Collision::AABB(Box b1, Box b2, float& moveX, float& moveY)
{
	moveX = moveY = 0;
...
Box Collision::getSweptBroadphaseBox(Box box, float deltaTime)
{
...
float Collision::sweptAABB(Box b1, Box b2, float& normalx, float& normaly, float deltaTime)
{
